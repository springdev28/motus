...
export function MotusStudio() {
  ...
  >({});
  const [layerTreeFocusId, setLayerTreeFocusId] = useState('');
  const [selectedElementId, setPrimarySelectedElementIdState] = useState('');
  const [selectedElementIds, setSelectedElementIds] = useState<string[]>([]);
  const layerSelectionScenes = useRef<{
    activeSceneId: string;
    elementsBySceneId: ReadonlyMap<string, readonly MotusElement[]>;
  }>({ activeSceneId: 'untitled-work-scene-1', elementsBySceneId: new Map() });
