...
The dedicated Motus project is `cgwfvxyfsoigzgbsiamn` in springdev28’s Org, Frankfurt. The initial schema was applied on September 11, 2026. Its public URL and publishable key are configured in Hostinger and the ignored local environment file. The account screen uses email and password with immediate signup and no recovery flow, as requested on September 12, 2026. Confirm email is off: the live Auth settings endpoint was verified to return `mailer_autoconfirm: true` and `disable_signup: false`. No SMTP sender or email-readiness variable is needed for this mode. The simplified screen passed 297 tests, type checking, lint and the production build. An authorized disposable live account verified immediate signup, profile creation and editing, sign-out, and sign-in with the saved profile on September 12, 2026. The test account and its profile and sessions were removed, with zero remaining records verified.
